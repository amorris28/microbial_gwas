...
functions. There are numerous examples where microbial diversity is correlated
with the rate of an ecosystem function, but exactly how microbial communities
“map” onto ecosystem functions remains unresolved.  This uncertainty limits our
ability to predict and manage crucial microbially mediated processes such as
nutrient loss and greenhouse gas emissions. Our lack of understanding stems
from the relatively large diversity of microorganisms, the difficulty in
directly identifying functional groups, and our limited ability to manipulate
microbial community attributes. For this reason, we propose that integrating
traditional biodiversity-ecosystem function research with ideas from
genotype-phenotype mapping could provide the new perspective our discipline
needs. We identify two insights from genotype-phenotype mapping that could be
useful for microbial biodiversity-ecosystem function studies: the concept of
“agnostic” mapping and controlling for population stratification to identify
...
published data measuring methane oxidation rates from tropical soils. We assert
that combining the approaches of traditional biodiversity-ecosystem function
research with ideas from genotype-phenotype mapping will not only generate
novel hypotheses about how complex microbial communities drive ecosystem
function, but also help scientists predict and manage changes to ecosystem
functions resulting from human activities.
...
performs this process). For example, the gene _mcrA_, which encodes a subunit of
...
examples include _pmoA_ and methanotrophy, _nifH_ and nitrification, and _nosZ_ and
...
abundance of _mcrA_ is often hypothesized to be related to the rate of
methanogenesis). 
...
agricultural ecosystems and for certain functions within the nitrogen cycle
[@rocca2015]. In general, including aspects of microbial biodiversity
(e.g., functional gene abundance or diversity) improve models of ecosystem
function less than one third of the time, and increase variance explained by an
...
that control variation in that phenotype [@hindorff2009, reich2001]. In
...
covariation. There is often no strong expectations about which entities (i.e.,
...
Beyond finding new physiologies in unexpected places, there are other reasons
...
that methanogens depend on. Or there could be indirect limitation by organisms
that liberate nitrogen or phosphorus into mineral forms. In other words, the
influence of microbial communities on the rate ecosystem function could
represent a complex metabolic network much like the regulation of gene
expression in organisms that partially determine their phenotype. These broader
patterns of biodiversity-ecosystem function relationships would be invisible to
any study that focuses on putative functional groups without considering the
reads that underlie those functional groups.
...
ancestry, and it could be very useful to account for these separately in
microbial studies, especially if one is particularly interested in how
composition alters function independent of the underlying environmental
variation.
...
that happen to have a high rate of ecosystem function. We would not be able to
distinguish these outcomes with this simple correlation. If we then include
environmental variables or environmental similarity as a covariate, we could
start to ask, “Is the taxon uniquely associated with function independent of
the environment?” By “uniquely associated”, we mean those taxa associated with
function irrespective of environmental conditions, local community structure,
or spatial proximity. This is not only a more specific question than is usually
asked in microbial biodiversity-ecosystem function studies, but it is also a
more appropriate one, especially if one is interested in how to incorporate
microbial community data into ecosystem models. Finally, it is interesting on
its own to understand whether microbial communities are selected by the
underlying environmental conditions to produce a particular rate of ecosystem
function (the conduit) or whether there is variation in community composition
independent of the environment that explains some variation in ecosystem
function.
...
reanalysis, we do not intend to challenge the conclusions from that paper, but
...
methane cycling pathways. For this example, we will analyze data from just one
...
latitude and longitude, and four environmental covariates (soil moisture, bulk
density, carbon, and nitrogen).

Analyses were conducted in the R statistical environment using the phyloseq
package [@mcmurdie2013, @rcoreteam2019]. The relative abundances of ASVs were corrected using the variance
stabilizing transformation from DESeq2 [@love2014,
@mcmurdie2014]. We first test typical measures of
microbial structure including functional gene abundance and community richness,
which was estimated using the breakaway package [@willis2015]. We then demonstrate
significant covariation between community structure (estimated as Bray-Curtis
distance using vegan), environmental variation (euclidean distance), and
geographic distance (euclidean distance) using Mantel tests [@bray1957, @oksanen2019]. Finally, we
...
including different
covariates (environmental, geographic, and community) can result in different
conclusions about which taxa are associated with function, we fit this model
with and without random effects variance components for environmental
similarity, geographic site ID, and Bray-Curtis similarity. Significant taxa
were determined by controlling the false discovery rate at q-value < 0.05 [@storey2002].
Figures were created using ggplot2 [@wickham2019].
...
our case, methane oxidation rate was not significantly correlated with the pmoA
gene abundance or 16S read richness (Table 1, Figure 1). To demonstrate that
...
populations. This means that the presence or abundance of certain genetic
markers (taxa) will be elevated in specific ecosystems. In this case,
...
be correlated with methane oxidation regardless of their involvement in that
process. It is necessary to control for this structuring to rigorously identify
...
the false discovery rate, 460 unique 16S reads were identified as significantly
...
function mapping question. For example, controlling for environmental variation
tests for taxa that are related to function independent of environmental
conditions. Similarly, controlling for geographic distance tests for taxa that
are related to function independent of geographic proximity; geographic
proximity could influence function via unmeasured environmental variation that
is spatially structured or differences in dispersal history among sites due to
physical distance. In the model that controlled for all three covariates
(community, environment, and geography), only six ASVs were significantly
correlated with methane oxidation rate (Figure 3). These taxa could be useful
indicators for methane oxidation rate across space and different ecosystems. In
addition, researchers could manipulate their abundance in order to better
...
underlie biogeographical patterns [@martiny2006, @vellend2010]. It is also
...
asked in microbial biodiversity-ecosystem function studies, but it is also a
more appropriate one, especially if one is interested in how to incorporate
microbial community data into ecosystem models. These models depend on
identifying taxa that add explanatory power beyond that provided by other
factors, such as environmental conditions. Earth system models would benefit
most from microbial bioindicators that capture variation unexplained by other,
more easily measured variables (such as certain environmental conditions). The
approach we describe is an important step toward identifying such biomarkers.
...
summarize the probability of having a particular ecosystem function or rate.
This would be accomplished by measuring the abundance of the taxa identified in
an association study and determining their association with the rate of an
ecosystem function. Alternatively, the identified taxa could be incorporated
into a structural equation model in an attempt to better understand the
individual effects and interactions among taxa as they contribute to the rate
of ecosystem function. This might give an indication of the relative importance
of different taxa as compared to other factors, such as environmental
variables.

Ultimately, the relationships identified in any comparative mapping study
must be verified. For genotype-phenotype studies in organisms there are
multiple ways that this verification is
...
The most common way that loci identified in a genotype-phenotype mapping study are verified is
...
mapping studies. In the case of microbial biodiversity studies, it may be
possible to inhibit a particular functional group through the use of a specific
antimicrobial or a chemical inhibitor or by using phages with high
host-specificity [@maxson2016], but this is not generally possible. In some cases, we
may be able to isolate a microorganism of interest in pure culture and add it
back to an ecosystem, transiently increasing its abundance (roughly analogous
to “overexpressing” a gene). A greater focus on culture-based approaches could
increase the success of these kinds of microbial enrichments. Finally,
synthetic communities (contrived assemblages of microorganisms) may be the most
powerful way to test hypotheses about microbial biodiversity-ecosystem function
relationships, but currently these approaches are limited by the small number
of taxa that can be routinely cultured from most environments.
...
and others unique. For example,
simple linear models such as the variance component model used in this study
are typical for genetics studies, but may not be the best way to identify
correlations for microbiomes due to the unique challenges of microbial data.
These are zero-inflated compositional data with differences in sequencing
depth across samples. We have addressed these challenges by using a variance
stabilizing transformation, but other models that test differential
abundance and differential variance that can control for differences in
sequencing depth and are robust to zero-inflation might be more appropriate
(e.g., CORNCOB @martin2019).  Alternatively, clustering reads at higher
taxonomic levels could provide a more continuous variation in taxon abundances
across ecosystems by reducing the number of “bins.” The tradeoff for this is
that it introduces bias based on the completeness of our taxonomic databases
and the accuracy of 16S-based taxonomic assignment and it removes reads that
lack a taxonomic assignment. Here, we chose to test ASVs at the level of the
individual read so as not to bias our results by losing reads that lack a
taxonomic assignment at a particular level.

Experimentally, future studies could improve on our example by sampling a more
homogeneous set of ecosystems. Our survey includes grasslands, pastures,
forests, peatlands, and mineral soil wetlands among others. These ecosystems
represent a range of moisture conditions that could regulate the abundance and
activity of methane oxidizers and access to methane and oxygen, which methane
oxidizers rely on. While this captured substantial variation in methane
oxidation rates, sampling from such diverse ecosystems could result in spurious
associations between taxa and function. For example, taxa differentially
abundant in upland ecosystems that are unrelated to methane oxidation might
appear correlated simply due to their presence in those ecosystems with high
oxidation rates. Future studies could try restricting their search to a more
homogeneous population of ecosystems specific to the question at hand.
...
of microbial functional groups (as currently defined) are often not good
